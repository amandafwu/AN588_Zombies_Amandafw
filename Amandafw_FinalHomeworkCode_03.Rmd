---
title: "Amandafw_FinalHomeworkCode_03"
author: "Amanda Wu"
date: "`r Sys.Date()`"
...
h <- ggplot(data = d, aes(x = gender, y = height)) #ggplot requires entering the dataset and aes for the x and y plots
h <- h + geom_boxplot() #this makes it a box plot
h <- h + ylab("Height") + xlab("Gender") #renames the x and y axis titles
...
Notes: the graphs look a little condensed though and I'm not too sure how to fix it or whether I should change it so each graph has their own space, however, the knitted code looks really long when I do this
  + I decided to take the code out but this was what I had used to condense the graphs:
...
set.seed(1) #makes sure that it randomly generates one reponse
height_99 <- NULL 
for (i in 1:99) {
    height_99[i] <- mean(sample(d$height, size = 30, replace = FALSE))
} #taken from mod 9, gets 99 random samples; a loop for the 99 samples
all_h <- c(height_99, m_sh) #puts the 99 samples and the previous random sample from question 5 into a vector
mean(all_h) #calc mean
sd(all_h) #calc sd 
...
## Challenges in this code: 
  + One of the biggest one was in question 6 where I didn't know how to combine the samples but it was almost an obvious solution once someone else explained it to me, I just didn't remember how to apply the older concepts into my own code
  + I also originally had difficulty understanding how to calculate the population vs sample sd; if there wasn't a note in the question I probably would've done sd() but because the question clarified that sd() is for samples, I had to look further into the modules to understand the concepts
  + I also had difficulty uploading the dataset into R at first because it kept saying that the final line of code was incomplete? I tried using different methods of uploading data into R from module 3 until I figured out the correct solution
  + This homework assignment was super repetitive so the knitted version ended up being super long and I wanted to find a way of condensing the graphs. This led me to a new package found from an online source which helped me group all the ggplots together but I ended up not including it in my final code because it wasn't the most visually appealing and made the graphs a bit hard to read since it was condensed. If I had more free time to work on this assignment, I might've found a solution to this problem but for now my knitted html will unfortunately be super long :(
  + I don't think I really understood the concepts of CI in the modules or even in class but doing this homework assignment really helped me better understand the topics by actually applying them into other situations where I am forced to find the answer; it took some time and googling but I think I was able to better grasp this topic after finishing this assignment :) 
)